...
import Sidebar from '../components/Sidebar';
import '../styles/MyPage.css';
import { teamData } from '../components/TeamData'; // 팀 데이터 파일에서 불러오기

function MyPage() {
    const [nickname, setNickname] = useState(localStorage.getItem('nickname') || '');
    const [team, setTeam] = useState(localStorage.getItem('selectedTeam') || '');
    const [teams, setTeams] = useState(teamData); // 기본 팀 데이터를 상태로 사용
    const [nicknameError, setNicknameError] = useState('');
    const [darkMode, setDarkMode] = useState(false);
    const [profileImage, setProfileImage] = useState('/logo192.png');
    const [isLoggedIn, setIsLoggedIn] = useState(false);
    ...
    useEffect(() => {
        ...
        setProfileImage(localStorage.getItem('profileImage') || '/logo192.png');
    }, []);
                ...
                <div className="profile-settings">
                    <div className="setting-item">
                        ...
                        <select value={team} onChange={handleTeamChange}>
                            <option value="">팀을 선택하세요</option>
                            {teams.map((team) => (
                                <option key={team.id} value={team.teamCall}>
                                    {team.name}
                                </option>
                            ))}
                        </select>
